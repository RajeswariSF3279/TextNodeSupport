...
window.sfBlazor.Dialog = function () {
  ...
  var Dialog = {
    ...
    getMaxHeight: function getMaxHeight(dataId) {
      return dataId && window.sfBlazor.instances[dataId].getMaxHeight();
    },
    changePosition: function changePosition(dlgObj) {
      if (dlgObj.dataId) {
        window.sfBlazor.instances[dlgObj.dataId].changePosition(dlgObj);
      }
    },
    updateAnimation: function updateAnimation(dlgObj) {
      if (dlgObj.dataId) {
        window.sfBlazor.instances[dlgObj.dataId].updateContext(dlgObj);
      }
    },
    focusContent: function focusContent(dataId) {
      dataId && window.sfBlazor.instances[dataId].focusContent();
    },
    refreshPosition: function refreshPosition(dataId) {
      dataId && window.sfBlazor.instances[dataId].refreshPosition();
    },
    getDimension: function getDimension(element) {
      if (!sf.base.isNullOrUndefined(element)) {
        return element.blazor__instance.getDimension();
      }

      return null;
    },
    popupCloseHandler: function popupCloseHandler(dataId) {
      dataId && window.sfBlazor.instances[dataId].popupCloseHandler();
    },
    propertyChanged: function propertyChanged(changedProps) {
      if (changedProps.dataId) {
        window.sfBlazor.instances[changedProps.dataId].OnPropertyChanged(changedProps);
      }
    },
    show: function show(options) {
      if (options.dataId) {
        window.sfBlazor.instances[options.dataId].show(options.fullScreen, options.maxHeight);
      }
    },
    hide: function hide(dataId) {
      return dataId && window.sfBlazor.instances[dataId].hide();
    },
    destroy: function destroy(dlgObj) {
      if (dlgObj.dataId) {
        window.sfBlazor.instances[dlgObj.dataId].destroy(dlgObj, dlgObj.isClient);
      }
    }
  };
  return Dialog;
